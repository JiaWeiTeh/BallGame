...
dt = 1.5  #keep this constant, unless wanting to speed up time
...
class Ball:
    ...
    def update(self, dt):
        ...
        if (self.pos[0] >= barR.pos[0] - 4 and self.pos[0] < boardx) or\
            (self.pos[0] <= barL.pos[0] + 4 and self.pos[0] > 0):
                # check if position of ball is within height of bar
                if (self.pos[1] <= barR.pos[1] + barlength/2 and\
                    self.pos[1] >= barR.pos[1] - barlength/2) or\
                    (self.pos[1] <= barL.pos[1] + barlength/2 and\
                    self.pos[1] >= barL.pos[1] - barlength/2):
...
class BarLeft:
    def __init__(self, position, length):
        ...
        self.plt = np.array([[position[0], position[0]],\
                             [position[1]-barlength/2, position[1]+barlength/2]])
...
def animate(i):
    ...
    if ball.pos[0] > barR.pos[0] or ball.pos[0] < barL.pos[0]:
        if (ball.pos[0] > boardx and (ball.pos[1] > barR.pos[1] or ball.pos[1] < barR.pos[1])) or\
            (ball.pos[0] > boardx and (ball.pos[1] > barL.pos[1] or ball.pos[1] < barL.pos[1])):
